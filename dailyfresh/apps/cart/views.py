...
# 添加商品到购物车:
# 1：请求方式，采用ajax post
# 如果涉及到数据的修改(新增、删除、更新)，采用post
# 如果只涉及到数据的获取，采用get
# 2：传递参数：商品id(sku_id) 商品数量(count)


# /cart/add
# ajax发起的请求都在后台(不能返回一个新的网页)，在浏览器中看不到效果所以 用LoginRequiredMixin仅限用户登录的网页没有效果
class CartAddView(View):
    '''购物车记录添加'''
    def post(self, request):
        '''购物车添加'''
        user = request.user  # 获取用户的信息登录
        if not user.is_authenticated():  # 判断用户没有登录
            # 用户未登录
        ...
        # 先尝试获取sku_id的值-> hget cart_key 属性(sku_id)
        # 如果sku_id在hash中不存在，hget返回None
        cart_conn = conn.hget(cart_key, sku_id)
        if cart_conn:  # 如果存在
            # 累加购物车中的商品的数目
            count += int(cart_conn)
        # 校验商品的库存
...
class CartUpdateView(View):
    ...
    def post(self, request):
        ...
        # 3. 业务处理:更新购物车记录，链接redis数据库，并获取里面的用户id
        conn = get_redis_connection('default')
        cart_key = 'cart_%d' % user.id
        ...
        # 计算用户购物车中商品的总件数 {'1':5, '2':3} 5+3=8
        total_count = 0
        vals = conn.hvals(cart_key)  # 获取hash的所有key的value值
        for val in vals:
            total_count += int(val)
