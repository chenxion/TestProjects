...
from django.conf import settings  # 导入系统配置文件
...
class OrderPlaceView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        # 获取登录的用户
        user = request.user
        # 获取参数sku_ids POST返回一个QueryDict:一个键对应多个值
...
# 高并发:秒杀, 使用悲观锁
# 支付宝支付
class OrderCommitView_old(View):
    '''订单创建'''
    @transaction.atomic  # 被装饰的下面函数，会在一个mysql事务里面要么都成功执行，要么都失败
    def post(self, request):
        '''订单创建'''
        # 判断用户是否登录
        user = request.user
...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        try:  # 获取的订单满足4个条件(订单id,用户名,支付宝支付,未支付状态)，说明是有效的订单
        ...
        total_pay = order.total_price + order.transit_price  # total_price是Decimal字段是Json类型的数据需要str()转化
    ...
    def post(self, request):
        ...
        while True:  # 每过5秒循环查看支付状态
            ...
            if code == '10000' and response.get('trade_status') == 'TRADE_SUCCESS':
                ...
                # 业务处理失败，可能一会就会成功
                import time
                time.sleep(5)  # 循环等待5秒再重新获取用户的支付状态
                continue
            else:
                # 支付出错
                print(code)
...
class CommentView(LoginRequiredMixin, View):
    ...
    def get(self, request, order_id):
        ...
        # 动态给order增加属性order_skus, 保存订单商品信息(就是所有商品和小计)
    ...
    def post(self, request, order_id):
        ...
        order.order_status = 5  # 更改订单状态已完成
        order.save()
