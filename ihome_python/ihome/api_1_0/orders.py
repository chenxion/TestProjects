...
def get_user_orders():
    ...
    try:
        if "landlord" == role:
            # 以房东的身份查询订单
            # 先查询属于自己的房子有哪些：房屋的用户id如果等于查询者的id，将他的房屋显示出来
            houses = House.query.filter(House.user_id == user_id).all()
            houses_ids = [house.id for house in houses]
            # 再查询预订了自己房子的订单：订单的房屋id里面有我的房屋id的全部显示出来
            orders = Order.query.filter(Order.house_id.in_(houses_ids)).order_by(Order.create_time.desc()).all()
        else:
            # 以房客的身份查询订单， 查询自己预订的订单：订单的用户id和查询者id相同显示出来
    ...
    # 将订单对象转换为字典数据
    orders_dict_list = []  # 定义空列表
    if orders:
        for order in orders:
            orders_dict_list.append(order.to_dict())  # 把订单的详细信息添加到列表
...
def accept_reject_order(order_id):
    ...
    if action not in ("accept", "reject"):  # 参数必须是这两种的一种才可以
        return jsonify(errno=RET.PARAMERR, errmsg="参数错误")

    try:
        # 根据订单号查询订单是否存在，并且要求订单处于等待接单状态
    ...
    if action == "accept":
        ...
        if not reason:  # 如果没有拒单原因，返回错误
            return jsonify(errno=RET.PARAMERR, errmsg="参数错误")
        order.status = "REJECTED"
        order.comment = reason  # 把拒单原因添加到评论字段
...
def save_order_comment(order_id):
    ...
    try:
        # 需要确保只能评论自己下的订单，而且订单处于待评价状态才可以。   订单的用户id等于你的id
