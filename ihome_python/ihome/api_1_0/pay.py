...
from . import api
from ihome.utils.commons import login_required  # 登录验证装饰器
from ihome.models import Order, db
...
def order_pay(order_id):
    ...
    alipay_client = AliPay(
        appid="2016092500592382",
        app_notify_url=None, # 默认回调url
        app_private_key_path=os.path.join(os.path.dirname(__file__), "keys/app_private_key.pem"),  # 私钥
        alipay_public_key_path=os.path.join(os.path.dirname(__file__), "keys/alipay_public_key.pem"),  # 支付包公钥
        sign_type="RSA2",  # 加密方式
        debug= True
    )
    #                                           dev是沙箱环境
    ...
    order_string = alipay_client.api_alipay_trade_wap_pay(
        ...
        return_url="http://127.0.0.1:5000/orders.html",  # 返回的连接地址
        notify_url=None, # 可选
    )

    # 构建让用户跳转的支付连接地址
    pay_url = constants.ALIAPY_URL_PREFIX + order_string
...
def save_order_payment_result():
    ...
    # 对支付宝的数据进行分离，提取出支付宝的签名参数(sign)和剩下的其他数据
