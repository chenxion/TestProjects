...
class User(BaseModel, db.Model):
    ...
    houses = db.relationship("House", backref="user")  # 用户发布的房屋  关联字段方便查询
    orders = db.relationship("Order", backref="user")  # 用户下的订单  backref:泛解析回到user表

    # 加上property装饰器后，会把函数变为属性，属性名就是函数名
    @property
    def password(self):
        """读取属性的函数行为"""
        # 函数的返回值会做为属性值
    ...
    def check_password(self, passwd):
        '''
        检验密码的正确性
        :param passwd: 用户登陆是填写的原始密码
        :return: 如果正确，返回True，否则返回False
        '''
        return check_password_hash(self.password_hash, passwd)

    # 前端需要的用户信息，提前把数据库每一个用户信息转换为字典打包好，直接调用
    def to_dict(self):
        """将数据库对象转换为字典数据"""
        user_dict = {
            "user_id": self.id,
            "name": self.name,
            "mobile": self.mobile,
            #                      七牛的域名前缀+图片名字          如果图片名字存在返回左边，不存在返回 “”空
    ...
    # 前端需要的认证信息，提前把数据库每一个用户信息转换为字典打包好，直接调用
...
class House(BaseModel, db.Model):
    ...
    # 前端需要的房屋基本信息，提前把数据库每一个房屋信息转换为字典打包好，直接调用
    ...
    # 前端需要的房屋详细信息，提前把数据库每一个房屋信息转换为字典打包好，直接调用
...
class Order(BaseModel, db.Model):
    ...
    status = db.Column(  # 订单的状态
        ...
        default="WAIT_ACCEPT", index=True)  # 指明在mysql中这个字段建立这个索引，加快查询速度
    comment = db.Column(db.Text)  # 订单的评论信息或者拒单原因
    trade_no = db.Column(db.String(80))  # 交易的流水号，支付宝

    # 前端需要的订单信息，提前把数据库每一个订单信息转换为字典打包好，直接调用
    def to_dict(self):
        """将订单信息转换为字典数据"""
        order_dict = {
            "order_id": self.id,
